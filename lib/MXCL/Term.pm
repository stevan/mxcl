...
class MXCL::Term::Environment :isa(MXCL::Term) {
    ...
    method length { scalar keys %$entries }

    method has    ($key)         { exists $entries->{ $key } }
    method get    ($key)         { $entries->{ $key } }
    method set    ($key, $value) { $entries->{ $key } = $value }
    method delete ($key)         { delete $entries->{ $key } }
    ...
    method lookup ($key) {
        ...
        #use Data::Dumper; warn Data::Dumper::Dumper($entries);
        return;
    }
    ...
    method equals ($other) {
        return true if refaddr $self == refaddr $other;
        return false unless $other isa __CLASS__;
        return false unless $self->length == $other->length;
...
class MXCL::Term::Opaque :isa(MXCL::Term) {
    ...
    method stringify {
        return sprintf '(opaque ())' unless defined $env;
        return sprintf '(opaque %s)' => join ' ' => map {
            sprintf ':%s %s' => $_, $env->entries->{$_}->type
        } keys $env->entries->%*;
    }
